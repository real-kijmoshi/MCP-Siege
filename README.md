...
- Right-click raw resources to gather, open land to move, or enemies to attack.
...
- Press A to select all villagers, H for the Town Hall, F to center, or M to arm movement.
- Pan the map with `WASD` or arrow keys.
- Zoom with the mouse wheel.

Every action crosses the same deterministic command queue. WebMCP worker allocation is a secondary convenience that issues physical world-gathering orders.
...
The Vite server sends `Origin-Agent-Cluster: ?1` and `Permissions-Policy: tools=(self)`, which native WebMCP requires. WebMCP has no on-page panel, chatbot, connection widget, or development inspector; discovery and invocation happen in the browser agent.
